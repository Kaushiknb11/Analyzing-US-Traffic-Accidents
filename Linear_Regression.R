...
lm_model <- lm(Total_Impact ~ `Average EuroNCAP Lane Deviation Score` + `Driver Claim History Rating` + `No Year no claims` + Gender, data=training_set)

summary(lm_model)

# simple linear regression model
lm_model1 <- lm(Total_Impact ~ `Average EuroNCAP Lane Deviation Score` , data=training_set)

summary(lm_model1)

# Generate predicted values using the testing set
predictions <- predict(lm_model1, newdata = testing_set)

# Compare predicted values to actual values
library(ggplot2)
ggplot(data = testing_set, aes(x = `Average EuroNCAP Lane Deviation Score`, y = Total_Impact)) +
  geom_point() +
  geom_line(aes(x = `Average EuroNCAP Lane Deviation Score`, y = predictions), color = "red")

plot(lm_model1)


# Compare model 1 to model 2
anova(lm_model1, lm_model)
